;(function(){
  ...
  /*   */  window.__$scriptOnLoadBarriers[decodeURIComponent("wasmcode1%2Fgame.js")] = {
  /* C */    onDidOpen(fn) {
  ...
  ; /////////// End - B A R R I E R   C O D E ////////////// ;
    ...
    const doWhenAllScriptLoaded = () => {
      ...
      if (!babelLock && scriptCounter <= 0) {
        // 当前包全部脚本加载完
        timing.addCost('MAIN_PACKAGE_LOAD', loadStartTs, Date.now(),{packageName:`/wasmcode1/`})
        ...
        innerScript.text = `var decodePathName = decodeURI("wasmcode1/game.js");require(decodePathName)`
        head.appendChild(innerScript)
        timing.addCost('MAIN_PACKAGE_REQUIRE', requireStartTs, Date.now(),{packageName:`/wasmcode1/`})
        timing.addPoint('USERCODE_REQUIRED', Date.now())

        alert(`SUBPACKAGE_READY_/wasmcode1/`)
        finishUp()
      }
    }
    ...
    const scriptLoaded = function() {
      ...
      alert('SUBPACKAGE_PROGRESS_' + progress + `_35541615_/wasmcode1/`)
